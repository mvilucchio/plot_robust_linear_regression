...
# Fixed point imports
from src.fpeqs_L1 import var_func_L2, var_hat_func_L1_decorrelated_noise
from src.fpeqs import state_equations

# BO simulation imports
from src.numerics import measure_gen_decorrelated, find_coefficients_L1, _find_numerical_mean_std

def L1_fixed_point(lambd, alpha, delta_small, delta_large, percentage, beta):
...
def L1_sim(d, lambd, alpha, delta_small, delta_large, percentage, beta, repetitions=10):
    """Uses CVXPY for solving L1 ridge regression to find the generalisation error on syntetic data. It should match the output of L2_fixed_point
    ...
    mean, std = _find_numerical_mean_std(alpha, measure_gen_decorrelated, find_coefficients_L1, d, repetitions, params, (lambd,))
...
def main():
    ...
    E_FP = L1_fixed_point(lambd, alpha, delta_small, delta_large, percentage, beta)
    print(f"E (fixed point): {E_FP}")

    d = 100
    E_L1 =  L1_sim(d, lambd, alpha, delta_small, delta_large, percentage, beta, repetitions=10)
    print(f"E (L1): {E_L1}")
