...
delta_larges = np.logspace(np.log10(0.1), np.log10(300), N)
epsilons = np.logspace(-3, np.log10(0.799), N)
# epsilons = np.linspace(0.01, 0.2, N)
...
if numerics_new:
    ...
    np.savetxt(
        "./data/FIGURE_3_right_epsilons_delta_out_beta_1.csv",
    ...
    np.savetxt(
        "./data/FIGURE_3_right_a_opt_beta_1.csv",
        # a_hub,
        difference_hub_l2,
        delimiter=",",
    )
...
difference_hub_l2 = np.genfromtxt(
    "./data/FIGURE_3_right_a_opt_beta_1.csv",
    delimiter=",",
)
...
# difference_hub_l2 = np.vectorize(lambda x: np.log10(1e-10) if x < 1e-10 else np.log10(x))(difference_hub_l2)

epseps, deltadelta_out = np.meshgrid(epsilons, delta_larges)
cs = ax.contourf(
    epseps, deltadelta_out, difference_hub_l2.T, levels = 13#, levels=np.logspace(np.log10(min_val), np.log10(max_val), N_LINES)
)  # locator=ticker.LogLocator(), , levels=np.linspace(0.01,5,30)
ax.contour(
    epseps, deltadelta_out, difference_hub_l2.T, levels = 13,# levels=np.logspace(np.log10(min_val), np.log10(max_val), N_LINES), 
    colors='black', alpha=0.7, linewidths=0.5
)
cbar = fig.colorbar(cs)
# cbar.ax.set_ylabel(r"$\log_{10} ( E^{\ell_2}_{\text{gen}} - E^{\text{Huber}}_{\text{gen}} )$", labelpad=1.0)

# ax.set_xlabel(r"$\epsilon$", labelpad=1.0)
# ax.set_ylabel(r"$\Delta_\text{\tiny{OUT}}$", labelpad=1.0)
ax.set_xscale("log")
ax.set_yscale("log")
...
if save:
    pu.save_plot(
        fig,
        "FIGURE_3_right_phase_diagram_beta_1",
    )
